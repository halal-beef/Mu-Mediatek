  ...
  IN EFI_DEVICE_PATH_PROTOCOL    *RemainingDevicePath)
{
  ...
  //
  Status = gBS->OpenProtocol(Controller, &gMediatekKeypadDeviceProtocolGuid, (VOID **)&KeypadDevice, This->DriverBindingHandle, Controller, EFI_OPEN_PROTOCOL_BY_DRIVER);
  if (EFI_ERROR(Status)) {
    return Status;
  }
  ...
  //
  gBS->CloseProtocol(Controller, &gMediatekKeypadDeviceProtocolGuid, This->DriverBindingHandle, Controller);
